...
function addRecordingToSheet(report, callback) {
  ...
  if (report.TranscriptionStatus === 'failed') {
    ...
    followupText.body = `${LANG.voice.error.esp} ${APP.email}.`
  ...
  spread(report, (err, result) => {
    if (err)
      followupText.body = `${LANG.voice.error[language]} ${APP.email}.`
    else
      followupText.body = LANG.voice.confirmation[language]
